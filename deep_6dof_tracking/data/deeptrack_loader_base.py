...
class DeepTrackLoaderBase(LoaderBase):
    ...
    def add_pose(self, rgb, depth, pose, depthonly=False):
        index = self.size()
        frame = self.frame_class(rgb, depth, str(index))
        self.data_pose.append([frame, pose])
        return index
    
    def add_pose(self, rgb, depth, pose, mask=None, depthonly=False):
        index = self.size()
        frame = self.frame_class(rgb, depth, str(index), mask=mask)
        self.data_pose.append([frame, pose])
        return index
    ...
    def add_pair(self, rgb, depth, pose, id, depthonly=False):
        id_int = int(id)
    ...
    def dump_images_on_disk(self, verbose=False, pad_name=False):
        """
        ...
        for frame, pose in tqdm(self.data_pose):
            ...
            if int(frame.id) in self.data_pair:
                for pair_frame, pair_pose in self.data_pair[int(frame.id)]:
                    pair_frame.dump(self.root, pad_name=pad_name)
            frame.dump(self.root, pad_name=pad_name)
