...
class DeepTrackBinGeoLoader(DeepTrackLoaderBase):
    def __init__(self, root, pretransforms=[], posttransforms=[], target_transform=[],
                 n_bin=41, linear_bins=True, callback=None, model=None):
        self.n_bin = n_bin
        self.callback = callback
        self.linear_bins = linear_bins
        super(DeepTrackBinGeoLoader, self).__init__(root, pretransforms, posttransforms, target_transform)
        # compute Bins
    ...
    @staticmethod
    def smooth_targets(val):
        val = gaussian_filter1d(val, sigma=0.5, axis=1, order=0, mode='constant')
        val = gaussian_filter1d(val, sigma=0.5, axis=1, order=0, mode='constant')
