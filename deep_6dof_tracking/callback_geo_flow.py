...
class DeepTrackGeoFlowCallback(LoopCallbackBase):
    ...
    def batch(self, predictions, network_inputs, targets, isvalid=True):
        ...
        if len(predictions) == 2:
            mask_loss = F.mse_loss(Variable(predictions[1].data.clone()), Variable(targets[4])).data[0]
    ...
    def epoch(self, loss, data_time, batch_time, isvalid=True):
        ...
        losses = [sum(self.dof_losses)/len(self.dof_losses),
                  sum(self.dof_norm_losses)/len(self.dof_norm_losses),
