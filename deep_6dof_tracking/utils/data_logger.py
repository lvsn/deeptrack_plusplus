    ...
    def save(self, path, name=None):
        for key, df in self.data_frames.items():
            if name is None:
                ...
                filename = os.path.join(path, name + "_" + key + ".csv")
            if key == 'projection_error':
                mean_error = df.mean()  # Compute the mean of the 'error' column
                print(f"Mean of 'error' column for {key}: {mean_error}")
