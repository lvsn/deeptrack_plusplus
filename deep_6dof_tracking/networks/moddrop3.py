...
class ModDrop3(torch.nn.Module):
    def __init__(self, channel_groups, dropout_proba=0.5, conv_size=10, conv_stride=1, normalize_channels=True):
        ...
        self.merge_conv = nn.Sequential(nn.Conv2d(len(channel_groups)*conv_size, len(channel_groups)*conv_size, 1, 1),
                                        nn.ELU(inplace=True))
