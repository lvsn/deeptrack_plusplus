...
class DeepTrackGeoFlowNet(NetworkBase):
    def __init__(self, show_mask=False):
        super(DeepTrackGeoFlowNet, self).__init__()
    ...
    def forward(self, A_0, B_0):
        ...
        delta_pose_T = F.tanh(delta_pose[:, :3]) * 0.05
        delta_pose_R = F.tanh(delta_pose[:, 3:])
        delta_pose = torch.cat((delta_pose_T, delta_pose_R), 1)
        # Deconvolution
    ...
    def loss(self, predictions, targets):
        if DEBUG:
            delta_pose, model, previous_pose, K, optical_flow, debugA, debugB, _, _ = targets
        else:
            delta_pose, model, previous_pose, K, optical_flow, _, _ = targets
        pose_prediction = predictions[0]
        loss_decoder = 0
        if len(predictions) == 2:
            mask_prediction = predictions[1]
            loss_decoder = nn.MSELoss()(mask_prediction, optical_flow)
        #rescale the outputs
        """
        ...
        prediction_translation = pose_prediction[:, :3]
        prediction_rotation = pose_prediction[:, 3:]
        ...
        projected_R_gt_model = model_R_gt[:, :, :] / model_R_gt[:, 2, :].unsqueeze(1)
        projected_R_prediction_model = model_R[:, :, :] / model_R[:, 2, :].unsqueeze(1)
        projected_T_gt_model = model_T_gt[:, :, :] / model_T_gt[:, 2, :].unsqueeze(1)
        projected_T_prediction_model = model_T[:, :, :] / model_T[:, 2, :].unsqueeze(1)

        if DEBUG:
            import matplotlib.pyplot as plt
            for i in range(1):

                R_prediction = coord_to_image(projected_R_prediction_model[i])
                R_gt = coord_to_image(projected_R_gt_model[i])
                R_prediction[R_gt[:, :, 0] == 255, :] = [0, 255, 0]
                T_prediction = coord_to_image(projected_T_prediction_model[i])
                T_gt = coord_to_image(projected_T_gt_model[i])
                ...
                plt.subplot("323")
                plt.imshow(debugA[i].data.cpu().numpy().T)
                plt.subplot("324")
                plt.imshow(debugB[i].data.cpu().numpy().T)
                plt.subplot("325")
                plt.imshow(optical_flow[i].data.cpu().numpy()[0, :, :].T)
                plt.subplot("326")
                plt.imshow(optical_flow[i].data.cpu().numpy()[1, :, :].T)
                plt.show()
        ...
        loss_pose = mean_mse_T + mean_mse_R
