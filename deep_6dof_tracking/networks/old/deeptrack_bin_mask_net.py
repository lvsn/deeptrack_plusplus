...
class DimLogSoftMax(nn.Module):
    """
    Implementation of
    teaching compositionality to cnns : https://arxiv.org/abs/1706.04313
    with deeptrack
    """
...
def normalize_gradients(grad):
    grad = grad.clone() / torch.norm(grad, 2)
    return grad


class DeepTrackBinMaskNet(NetworkBase):
    def __init__(self, n_bins, log_softmax=True):
        super(DeepTrackBinMaskNet, self).__init__()
        self.n_bins = n_bins
        filter_size_1 = 24
        embedding_size = 500
    ...
    def forward(self, A, B, B_mask=None, mask0=None, mask1=None, mask2=None, mask3=None, mask4=None):
        ...
        if B_mask is not None:
            ...
            return o_ABL[:, 0, :], o_ABL[:, 1, :], o_ABL[:, 2, :], o_ABL[:, 3, :], o_ABL[:, 4, :], o_ABL[:, 5, :], \
                   m_ABL[:, 0, :], m_ABL[:, 1, :], m_ABL[:, 2, :], m_ABL[:, 3, :], m_ABL[:, 4, :], m_ABL[:, 5, :], \
                   o_B1, o_B2, o_AB1, o_AB2, o_AB3, \
                   m_B1, m_B2, m_AB1, m_AB2, m_AB3, m_ABL
        else:
            return o_ABL[:, 0, :], o_ABL[:, 1, :], o_ABL[:, 2, :], o_ABL[:, 3, :], o_ABL[:, 4, :], o_ABL[:, 5, :]
    ...
    def deep_track(self, A, B, mask0=None, mask1=None, mask2=None, mask3=None, mask4=None):
        ...
        ABL = self.dropout2(self.fc_bn1(F.elu(self.fc1(ABL))))
        ABL = self.fc2(ABL)
        ABL = ABL.view(-1, 6, self.n_bins)
        ABL = self.log_softmax(ABL, self.is_log_softmax)


        self.activations.append(B1)
        self.activations.append(B2)
        self.activations.append(AB1)
        self.activations.append(AB2)
        self.activations.append(AB3)

        return B1, B2, AB1, AB2, AB3, ABL

    def loss(self, predictions, targets):

        original_task_losses = []
        masked_task_losses = []
        activation_losses = []

        for i in range(6):
            original_task_losses.append(nn.KLDivLoss()(predictions[i], targets[i]))
            masked_task_losses.append(nn.KLDivLoss()(predictions[i+6], targets[i]))
        for i in range(5):
            prediction = torch.mul(predictions[12+i], targets[i+6].unsqueeze(1))  # mask the activation
            target = predictions[12 + i + 5]
            l = torch.sum((prediction - target) ** 2) / predictions[12+i].data.nelement()
            activation_losses.append(l)
        o_task_loss = sum(original_task_losses)
        m_task_loss = sum(masked_task_losses)

        activation_loss = activation_losses[0] * 0
        activation_loss += activation_losses[1] * 0
        activation_loss += activation_losses[2] * 0
        activation_loss += activation_losses[3] * 0.0005
        activation_loss += activation_losses[4] * 0.001
