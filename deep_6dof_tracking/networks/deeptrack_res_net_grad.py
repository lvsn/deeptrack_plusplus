...
class DeepTrackResNetGrad(NetworkBase):
    def __init__(self, image_size=150, phase=None):
        super(DeepTrackResNetGrad, self).__init__()

        filter_size_1 = 64
        embedding_size = 500
        half_filter_size_1 = int(filter_size_1 / 2)

        self.convA = nn.Conv2d(6, filter_size_1, 3, 2)
        ...
        self.convB = nn.Conv2d(6, filter_size_1, 3, 2)
    ...
    def stream(self, A, B, batch_norms):
        size = A.size(-1)
        gradient_x = np.linspace(0, 1, size, dtype=np.float32)[np.newaxis, :].repeat(size, axis=0) - 0.5
        gradient_y = np.linspace(0, 1, size, dtype=np.float32)[:, np.newaxis].repeat(size, axis=1) - 0.5

        minibatch_gradient_x = gradient_x[np.newaxis, np.newaxis, :, :].repeat(A.size(0), axis=0)
        minibatch_gradient_y = gradient_y[np.newaxis, np.newaxis, :, :].repeat(A.size(0), axis=0)

        gradient_x = torch.from_numpy(minibatch_gradient_x).cuda()
        gradient_y = torch.from_numpy(minibatch_gradient_y).cuda()

        A = torch.cat((A, gradient_x, gradient_y), dim=1)
        B = torch.cat((B, gradient_x, gradient_y), dim=1)
