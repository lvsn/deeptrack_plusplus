...
from deep_6dof_tracking.networks.moddrop2 import ModDrop2
...
class DeepTrackResNetModDrop2(NetworkBase):
    def __init__(self, image_size=150, phase=None, loss_func=nn.MSELoss()):
        super(DeepTrackResNetModDrop2, self).__init__()
        ...
        self.moddropB = ModDrop2([[0, 1, 2], [3]], conv_size=32, conv_stride=2, dropout_proba=0.66, normalize_channels=False)
    ...
    def stream(self, A, B, batch_norms):
        ...
        B = self.moddropB(B)
        fireB = self.fireB(B)
        self.probe_activation["moddrop"] = fireB
        B = self.batchB2(F.max_pool2d(torch.cat((fireB, B), 1), 2))
