...
class DeepTrackResNetModDropDeep(NetworkBase):
    def __init__(self, image_size=150, phase=None):
        super(DeepTrackResNetModDropDeep, self).__init__()
        ...
        self.moddropB = ModDrop([[0, 1, 2], [3]], dropout_proba=0.66, normalize_channels=False)
        ...
        self.fire3 = Fire(filter_size_1 * 16, filter_size_1 * 4, filter_size_1 * 2, filter_size_1 * 2)
        self.batch3 = nn.BatchNorm2d(filter_size_1 * 20)
        self.fire4 = Fire(filter_size_1 * 20, filter_size_1 * 4, filter_size_1 * 2, filter_size_1 * 2)
        self.batch4 = nn.BatchNorm2d(filter_size_1 * 24)

        view_width = int(int(int(int(int((image_size - 2)/2)/2)/2)/2)/2/2)
    ...
    def stream(self, A, B, batch_norms):
        ...
        AB = torch.cat((A, B), 1)
        AB = self.batch1(F.max_pool2d(torch.cat((self.fire1(AB), AB), 1), 2))
        AB = self.batch2(F.max_pool2d(torch.cat((self.fire2(AB), AB), 1), 2))
        AB = self.dropout_AB0(self.batch3(F.max_pool2d(torch.cat((self.fire3(AB), AB), 1), 2)))
        AB = self.dropout_AB1(self.batch4(F.max_pool2d(torch.cat((self.fire4(AB), AB), 1), 2)))
